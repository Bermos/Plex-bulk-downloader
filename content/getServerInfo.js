...
user = JSON.parse(window.localStorage.users).users.filter(
  (user) => typeof user.authToken !== "undefined"
)[0];
server = user.servers.filter(
...
token =
  typeof server.accessToken !== "undefined"
    ? server.accessToken
    : user.authToken;
...
if (detailsPath.includes("library/metadata")) detailsPath += "/children";
else if (detailsPath.includes("playlists")) detailsPath += "/items";

// find selected images/music
selectedItems = [];
try {
  selections = document.body.innerHTML.match(
    /(?<=class=")(MetadataTableRow-alternateItem-\d+-\w+\s+)?(MetadataTableRow-item-\w+-\w+\s+)?(MetadataTableRow-hasActiveSelection-\w+\s+)(MetadataTableRow-isSelected-\w+\s+)(?=")/g
  );
  ...
  selections.forEach(function (domClass) {
    ...
    for (i = 0; i < elements.length; i++) {
      ...
      const trackNumber = parseInt(
        [...element.children].filter((x) => x.tagName === "DIV")[0].firstChild
